<?php


class Template {

    private $template;
    private $controller;
    private $layouts;
    private $vars = array();

    function __construct($layouts, $controllerName) {
        $this->layouts = $layouts;
        $arr = explode('_', $controllerName);
        $this->controller = strtolower($arr[1]);
    }

    //Set parameters for layout
    function vars($varname, $value) {
        if (isset($this->vars[$varname]) == true) {
            trigger_error('Unable to set var' . $varname);
            return false;
        }
        $this->vars[$varname] = $value;
        return true;
    }

    // View
    function view($name) {
        $pathLayout = SITE_PATH . 'views' . DS . 'layouts' . DS . $this->layouts . ".php";
        $contentPage = SITE_PATH . 'views' . DS . $this->controller . DS . $name . ".php";

        // Check layout and template
        if (file_exists($pathLayout) == false) {
            trigger_error('The layout'. $this->layouts . 'does not exist', E_USER_NOTICE);
            return false;
        }
        if (file_exists($contentPage) == false) {
            echo $contentPage;
            trigger_error('The template'. $name . 'does not exist', E_USER_NOTICE);
            return false;
        }

        foreach ($this->vars as $key => $value) {
            $$key = $value;
        }

        require ($pathLayout);
    }
}